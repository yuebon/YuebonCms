				...
				if($("#goodsSpecBox dl dd a.selected").length == $("#goodsSpecBox dl").length){
					...
					$.ajax({
						...
						error: function (XMLHttpRequest, textStatus, errorThrown) {
							weui.alert("查询出错：" + textStatus + ",提示：" + errorThrown);
						}
					});
				}
			}
...
//打开规格选择窗口
function showActionSheet(actiontype, webpath, linkurl){
    //显示窗口
...
function cartAdd(actiontype, webpath, linkurl) {
	...
	if(actiontype == "buy"){
		...
		$.ajax({
			...
			success: function(data, textStatus) {
				if (data.status == 1) {
					weui.confirm('购物车共有' + data.quantity + '件商品，合计：' + data.amount + '元，是否去结算？', function(){
                        location.href = linkurl;
                    },function(){ },{
                        title: '放入购物车成功'
                    });
...
function updateCart(obj, webpath, num){
	...
	if(isNaN(goodsQuantity)){
		weui.alert('商品数量只能输入数字!');
		return false;
	}
	if(isNaN(stockQuantity)){
		weui.alert('检测不到商品库存数量!');
		return false;
	}
	if(goodsQuantity < 1){
		goodsQuantity = 1;
...
function initUserAddress(obj){
	...
	$(obj).children("label").each(function() {
		...
		//初始化事件
        $(this).find("input[name='user_book_id']").click(function(){
			$("#book_id").val($(this).parents("label").find("input[name='user_book_id']").val());
			$("#accept_name").val($(this).parents("label").find("input[name='user_accept_name']").val());
			$("#address").val($(this).parents("label").find("input[name='user_address']").val());
			$("#mobile").val($(this).parents("label").find("input[name='user_mobile']").val());
			$("#telphone").val($(this).parents("label").find("input[name='user_telphone']").val());
			$("#email").val($(this).parents("label").find("input[name='user_email']").val());
			$("#post_code").val($(this).parents("label").find("input[name='user_post_code']").val());
			//改变省市区
			var areaArr = $(this).parents("label").find("input[name='user_area']").val().split(",");
			if (areaArr.length == 3) {
				mypcas.SetValue(areaArr[0], areaArr[1], areaArr[2]);
			} else {
				mypcas.SetValue();
