...
namespace YueBonSoft.Web.tools
{
    /// <summary>
    /// submit_ajax 的摘要说明
    /// </summary>
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                case "article_category_list": //获取分类
                    article_category_list(context);
                    break;
                ...
                case "user_address_default": //设置用户默认收货地址
                    ...
                    break;
                case "user_goodsopportunity_edit": //编辑货源信息
                    user_goodsopportunity_edit(context);
                    break;
                ...
                case "user_address_delete": //删除用户收货地址
                    ...
                    break;
                case "user_goodsopportunity_delete": //删除货源
                    user_goodsopportunity_delete(context);
                    break;
                case "user_goodsopportunity_status": //设置货源为“已交易”
                    user_goodsopportunity_status(context);
                    break;
                ...
                case "view_article_click": //统计及输出阅读次数
                    ...
                    break;
                case "redo_article_count": //统计及输出转发次数
                    redo_article_count(context);
                    break;
                case "pengyouquan_article_count": //统计及输出分享发次数
                    pengyouquan_article_count(context);
                    break;
                case "redo_article_count_add": //增加转发次数
                    redo_article_count_add(context);
                    break;
                case "pengyouquan_article_count_add": //增加分享次数
                    pengyouquan_article_count_add(context);
                    break;
                ...
                case "view_cart_count": //输出当前购物车总数
                    ...
                    break;
                case "get_weixin_jssdk": //输出当前购物车总数
                    get_WeixinJSSDK(context);
                    break;
                    
            }
        }        

        #region 获取频道内容分类的处理方法OK===========================
        private void article_category_list(HttpContext context)
        {
            ...
            int parent_id = DTRequest.GetQueryInt("parent_id");
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string companyName = Utils.ToHtml(DTRequest.GetFormString("txtCompanyName"));
        ...
        #region 编辑用户货源信息OK=============================
        private void user_goodsopportunity_edit(HttpContext context)
        {
            try
            {
                int site_id = DTRequest.GetQueryInt("site_id"); //当前站点ID
                ...
                if (id > 0)
                {
                    if (!bll.Exists(7, id, userModel.user_name))
                    {
                        context.Response.Write("{\"status\":0, \"msg\":\"对不起，收货地址不存在或已删除！\"}");
                        return;
                    }
                ...
                string category_id = Utils.ToHtml(DTRequest.GetFormString("txtCategoryId"));
                string expressLine = Utils.ToHtml(DTRequest.GetFormString("txtExpressLine"));
                string goodsWeight = Utils.ToHtml(DTRequest.GetFormString("txtGoodsWeight"));
                string goodsVolume = Utils.ToHtml(DTRequest.GetFormString("txtGoodsVolume"));
                string expectMoney = Utils.ToHtml(DTRequest.GetFormString("txtExpectMoney"));
                string sendDate = Utils.StrToDateTime(DTRequest.GetFormString("txtsendDate")).ToString();
                string content = Utils.ToHtml(context.Request.Form["txtcontent"]);
                string contact = Utils.ToHtml(context.Request.Form["txtcontact"]);
                string companyname = Utils.ToHtml(context.Request.Form["txtcompanyname"]);
                string mobile = Utils.ToHtml(DTRequest.GetFormString("txtMobile"));
                ...
                {
                    context.Response.Write("{\"status\":0, \"msg\":\"对不起，请选择货源分类！\"}");
                    return;
                }
                //检查手机
                if (string.IsNullOrEmpty(mobile))
                ...
                model.user_name = userModel.user_name;
                model.title = title;
                model.category_id = Utils.StrToInt(category_id, 0);
                model.channel_id = 7;
                //内容摘要提取内容前255个字符
                model.zhaiyao = Utils.DropHTML(content, 250);
                model.content = content;
                model.zhaiyao = expressLine + ",联系人："+ contact+",联系电话："+mobile;
                //model.is_msg = 0;
                //model.is_top = 0;
                //model.is_red = 0;
                //model.is_hot = 0;
                ...
                dic.Add("ExpressLine", expressLine);
                dic.Add("GoodsWeight", goodsWeight);
                dic.Add("GoodsVolume", goodsVolume);
                dic.Add("ExpectMoney", expectMoney);
                dic.Add("sendDate", sendDate);
                dic.Add("contact", contact);
                dic.Add("address", address);
                dic.Add("mobile", mobile);
                dic.Add("companyname", companyname);
                dic.Add("telphone", telphone);
                dic.Add("sub_title", "");
                dic.Add("source", "");
                dic.Add("author", userModel.user_name);
                model.fields = dic;
                if (id > 0)
                {
                    new BLL.article().Update(model);
                    send_addarticle_email(sitepath, model);
                    context.Response.Write("{\"status\":1, \"msg\":\"修改货源成功！\"}");
                }
                else
                {

                    model.click = 1;
                    dic.Add("ForwardNum", "0");
                    dic.Add("sharenum", "0");
                    model.add_time = DateTime.Now;
                    new BLL.article().Add(model);
                    send_addarticle_email(sitepath, model);
                    context.Response.Write("{\"status\":1, \"msg\":\"新增货源成功！\"}");
                }
            }catch(Exception e)
            {
                string msg = e.Message;
                context.Response.Write("{\"status\":01, \"msg\":\""+ msg + "\"}");
            }
            return;
        }
        #endregion
        ...
        #region 删除用户货源信息OK=============================
        private void user_goodsopportunity_delete(HttpContext context)
        {
            //检查用户是否登录
            Model.users model = new BasePage().GetUserInfo();
            if (model == null)
            ...
            string check_id = DTRequest.GetFormString("checkId");
            int channelid = Int32.Parse(DTRequest.GetFormString("channelid")); 
            if (string.IsNullOrEmpty(check_id))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"删除失败，请检查传输参数！\"}");
                return;
            ...
            for (int i = 0; i < arrId.Length; i++)
            {
                ...
                if (id > 0)
                {
                    new BLL.article().Delete((Int32)channelid,id);
                }
            }
            context.Response.Write("{\"status\":1, \"msg\":\"删除货源成功！\"}");
            return;
        }
        #endregion


        #region 修改用户货源状态信息-已交易OK=============================
        private void user_goodsopportunity_status(HttpContext context)
        {
            int site_id = DTRequest.GetQueryInt("site_id"); //当前站点ID
            string sitepath = new BLL.sites().GetBuildPath(site_id); //当前站点目录
            //检查用户是否登录
            ...
            for (int i = 0; i < arrId.Length; i++)
            {
                ...
                if (id > 0)
                {
                    Model.article model = new Model.article();
                    BLL.article bll = new BLL.article();
                    model = bll.GetModel(channelid,id);
                    model.status = 3;
                    new BLL.article().Update(model);
                    send_addarticle_email(sitepath, model);
                }
            }
            context.Response.Write("{\"status\":1, \"msg\":\"设置货源已交易成功！\"}");
            return;
        }
        #endregion
        #region 获取对应的商品信息OK===========================
        ...
        #region 统计及输出转发次数OK===========================
        private void redo_article_count(HttpContext context)
        {
            int channel_id = DTRequest.GetInt("channel_id", 0);
            int article_id = DTRequest.GetInt("id", 0);
            int count = 0;
            ...
            {
                BLL.article bll = new BLL.article();
                count = bll.GetForwardNum(channel_id, article_id);
            }
            context.Response.Write("document.write('" + count + "');");
        }
        #endregion
        ...
        #region 统计及输出分享次数OK===========================
        private void pengyouquan_article_count(HttpContext context)
        {
            int channel_id = DTRequest.GetInt("channel_id", 0);
            int article_id = DTRequest.GetInt("id", 0);
            int count = 0;
            ...
            {
                BLL.article bll = new BLL.article();
                count = bll.GetShareNum(channel_id, article_id);
            }
            context.Response.Write("document.write('" + count + "');");
        }
        #endregion
        ...
        #region 增加转发次数OK===========================
        private void redo_article_count_add(HttpContext context)
        {
            int channel_id = DTRequest.GetInt("channel_id", 0);
            int article_id = DTRequest.GetInt("id", 0);
            int click = DTRequest.GetInt("click", 0);
            if (channel_id > 0 && article_id > 0)
            {
                ...
                if (click > 0)
                {
                    bll.UpdateField(channel_id, article_id, "ForwardNum=ForwardNum+1");
                }
            }
        ...
        #region 增加分享次数OK===========================
        private void pengyouquan_article_count_add(HttpContext context)
        {
            int channel_id = DTRequest.GetInt("channel_id", 0);
            int article_id = DTRequest.GetInt("id", 0);
            int click = DTRequest.GetInt("click", 0);
            if (channel_id > 0 && article_id > 0)
            {
                ...
                if (click > 0)
                {
                    bll.UpdateField(channel_id, article_id, "sharenum=sharenum+1");
                }
            }
            context.Response.Write("{\"Success\":true}");
        }
        ...
        private string send_verify_sms_code(HttpContext context,string mobile)
        {
            ...
            Model.sms_template smsModel = new BLL.sms_template().GetModel("232177"); //取得短信内容
            if (smsModel == null)
            {
            ...
            //msgContent = msgContent.Replace("{valid}", userConfig.regsmsexpired.ToString());
            //msgContent[0] = strcode;
            //msgContent[1] =userConfig.regsmsexpired.ToString();
            //发送短信
            string tipMsg = string.Empty;
            bool result = new BLL.sms_message().SendSMS(mobile, smsModel.call_index, msgContent);
            if (!result)
            {
        ...
        #region 会员新增内容发送邮件通知管理员OK============================
        private string send_addarticle_email(string sitepath, Model.article model)
        {
            //获得邮件内容
            Model.mail_template mailModel = new BLL.mail_template().GetModel("newsgoodsopportunity");
            if (mailModel == null)
            {
                return "{\"status\":0, \"msg\":\"邮件发送失败，邮件模板内容不存在！\"}";
            }
            ...
            titletxt = titletxt.Replace("{webname}", sysConfig.webname);
            titletxt = titletxt.Replace("{username}", model.user_name);
            bodytxt = bodytxt.Replace("{title}", model.title);
            bodytxt = bodytxt.Replace("{add_time}", model.add_time.ToString());
            bodytxt = bodytxt.Replace("{webname}", sysConfig.webname);
            bodytxt = bodytxt.Replace("{webtel}", sysConfig.webtel);
            bodytxt = bodytxt.Replace("{weburl}", sysConfig.weburl);
            bodytxt = bodytxt.Replace("{username}", model.user_name);
            bodytxt = bodytxt.Replace("{linkurl}", "http://" + HttpContext.Current.Request.Url.Authority.ToLower()
                + new Web.UI.BasePage().getlink(sitepath, new Web.UI.BasePage().linkurl("goodsopportunity_show", model.id)));
            //发送邮件
            try
            {
                DTMail.sendMail(sysConfig.emailsmtp, sysConfig.emailssl,
        ...
        /// <returns></returns>
        private void get_WeixinJSSDK(HttpContext context)
        {
            int weixinId = DTRequest.GetQueryInt("weixin_id");
            BLL.weixin_account wxBll = new BLL.weixin_account();
            Model.weixin_account modelt = wxBll.GetModel(weixinId);
            var jssdkUiPackage = JSSDKHelper.GetJsSdkUiPackage(modelt.appid, modelt.appsecret, context.Request.Url.AbsoluteUri);
           
            context.Response.Write(JsonConvert.SerializeObject(jssdkUiPackage, Formatting.Indented));
            return ;
        }
