...
namespace YueBonSoft.DAL
{
	...
	public partial class article
	{
        ...
        /// 是否存在该记录
        /// </summary>
        public bool Exists(string channel_name, int article_id, string user_name)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=@article_id and user_name=@user_name");
            SqlParameter[] parameters = {
                    new SqlParameter("@article_id", SqlDbType.Int,4),
        ...
        /// 获取阅读次数
        /// </summary>
        public int GetForwardNum(string channel_name, int article_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ForwardNum from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=" + article_id);
            string str = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(str))
            {
        ...
        /// 获取分享次数（朋友圈）
        /// </summary>
        public int GetShareNum(string channel_name, int article_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 sharenum from " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name);
            strSql.Append(" where id=" + article_id);
            string str = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(str))
            {
