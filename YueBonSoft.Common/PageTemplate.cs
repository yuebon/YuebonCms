...
namespace YueBonSoft.Common
{
    ...
    public abstract class PageTemplate
    {
        ...
        static PageTemplate()
        {
            ...
            //时间字符串转时间类型
            r[23] = new Regex(@"(\{strtodate\(([^\s]+?)\)\})", options);
        }
            ...
            using (StreamReader objReader = new StreamReader(templetFullPath, Encoding.UTF8))
            {
                ...
                if (nest == 1)
                {
                    ...
                    string template = string.Format("<%@ Page Language=\"C#\" AutoEventWireup=\"true\" Inherits=\"{0}\" ValidateRequest=\"false\" %>\r\n" +
                        ...
                        "<%@ Import namespace=\"YueBonSoft.Common\" %>{1}\r\n\r\n" +
                        "<script runat=\"server\">\r\noverride protected void OnInit(EventArgs e)\r\n" +
                        "{{\r\n\r\n\t/* \r\n\t\tThis page was created by YueBoncms Template Engine at {2}.\r\n\t\t" +
                        "本页面代码由YueBoncms模板引擎生成于 {2}. \r\n\t*/\r\n\r\n\t{3}base.OnInit(e);\r\n\t" +
        ...
        private static string ConvertTags(int nest, string channelName, string pageSize, string sitePath, string tempPath, string skinName, string inputStr)
        {
            ...
            #region 时间字符串转时间类型================================================OK
            foreach (Match m in r[23].Matches(strTemplate))
            {
                IsCodeLine = true;
                strTemplate = strTemplate.Replace(m.Groups[0].ToString(),"Utils.ObjectToDateTime("+ m.Groups[2] + ")");
            }
            #endregion
            #region 字符串转换整型==================================================OK
